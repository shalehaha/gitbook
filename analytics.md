...
[Track App States in Android](#track-app-states-in-android)

[Track App Actions in Android](#track-app-actions-in-android)

[Track States and Actions in Android](#track-states-and-actions-in-android)

[Track App Crashes in Android](#track-app-crashes-in-android)

[Hit Batching in Android](#hit-batching-in-android)
...
[Analytics Methods in Android](#analytics-methods-in-android)

[Track App States in iOS](#track-app-states-in-ios)

[Track App Actions in iOS](#track-app-actions-in-ios)

[Track States and Actions in iOS ](#track-states-and-actions-in-ios)

[Track App Crashes in iOS ](#track-app-crashes-in-ios)

[Hit Batching in iOS](#_Toc513631201)

[Products Variable](#products-variable-1)

[Products Variable with Merchandising eVars and Product-Specific Events
](#products-variable-with-merchandising-evars-and-product-specific-events-1)

[Event Serialization](#event-serialization-1)

[Video Analytics](#video-analytics-1)

[Analytics Methods in iOS](#analytics-methods-in-ios)
...
| Product                           | Description |
|-----------------------------------|-----------------------------------|
| Adobe Mobile Services             | The **View States** report.  <br> This report is based on the paths that the users took through your  application. A sample path is    **Home** \>  **Settings** \> **Feed**.         |
...
![](./media/image2.jpeg)
 You can also set a prop to hold all action values by mapping a customprop with a name like **Custom Actions** and setting the value to a.action.
![](./media/image3.png)
...
| **Interface**                     | **Report**                        |
|-----------------------------------|-----------------------------------|
| Adobe Mobile Services             | **Action Paths** report.  <br>    View the order in which actions   occur in your app. You can also    click **Customize** on any report  to view actions ranked, trended,  or in a breakdown report or apply  a filter to view actions for a  specific segment.                 |
| Marketing reports & analytics     | **Custom Event** report.  <br>    After an action is mapped to a  custom event, you can view mobile  events similar to all other  Analytics events.                 |
| Ad hoc analytics                  | **Custom Event** report.    <br>      After an action is mapped to a  custom event, you can view mobile events similar to all other  Analytics events.                 |
...
>
> ![](./media/image5.png) ***Prerequisite:** App crashes are
...
> ![](./media/image5.png)***Tip:** You can avoid this crash by
...
>
> ![](./media/image7.png) ***Important:** You need to rely on
...
To set the *products* variable, set a context data key to "&&products", and set the value by using the syntax that is definedfor the *products* variable:
...
```
 ...
 The *products* variable is set on the image request, and the othervariables are set as context data:
...
In Adobe Mobile services, the state name is reported in the *View State* variable, and a view is recorded for each track\_state\_request.state call. In other Analytics interfaces, **View State** is reported as **Page Name**, and state views is reported as page views.

4.  In addition to the state name, you can send additional context data with each track state call:
...
3.  When the action that you want to track occurs in your app, call track\_action\_request to send a hit for this action.
 ...
 ![](./media/image50.png) **Tip:** If the code where you areadding this call might run while the app is in the background, call **trackActionFromBackground** instead of **track\_action\_request**.
...
>
> ![](./media/image7.png) ***Important:** You should upgrade to
...
> ![](./media/image50.png)***Tip:** You can avoid a crash in
...
> ![](./media/image50.png) ***Tip:** Background activity that
...
When the value is greater than 0, the SDK queues the number of hits equal to the *analytics.batchLimit* value. After this threshold is passed, all hits in the queue are sent.
...
To set the *products* variable, set a context data key to "&&products", and set the value by using the syntax that is defined for the variable:
...
![](./media/image50.png)***Tip:** If you trigger a product-specific event by using the &&products variable, you must also set that event in the &&events variable. If you do not set that event, it is filtered out during processing.*
